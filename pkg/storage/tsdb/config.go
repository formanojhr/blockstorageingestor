...
func (cfg *TSDBConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.StringVar(&cfg.Dir, "blocks-storage.tsdb.cmd", "tsdb", "Local directory to store TSDBs in the ingesters.")
...
func (cfg *BucketStoreConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.StringVar(&cfg.SyncDir, "blocks-storage.bucket-store.sync-cmd", "tsdb-sync", "Directory to store synchronized TSDB index headers.")
